...
[x] Extend `DesktopWidgetsTests/WidgetRenderingSmokeTests.swift` with a deterministic synthetic-data showcase rendered from the real widget views into temporary storage, and add an explicit generator script as the only path that refreshes the tracked preview.
