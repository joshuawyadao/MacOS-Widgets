...
3. Add **Time & Date** from the macOS widget gallery.
...
Each font choice includes a square `Aa` specimen rendered with that macOS font. The specimen is attached to its dynamic option as a template image, allowing macOS to tint it appropriately in light and dark menus. Font choices are separated into **Clean & Classic** and **Handwritten** sections. Apple controls the Edit Widget interface, including whether and at what size it displays those images; option titles themselves remain in the system interface font.
