...
The shared Xcode Run scheme executes `Scripts/refresh-widget-runtime.sh` before launching the app. It finds registrations for the current extension and known retired Desktop Widgets identifiers, unregisters stale copies inside Xcode's DerivedData tree, and removes a retired-ID database record at the exact current extension path before re-registering the current identifier. It then stops the existing Desktop Widgets extension process, registers the newly built extension, and restarts the current user's widget daemon. This prevents an existing desktop widget from remaining attached to another worktree's older executable, descriptor, or iconless gallery entry. It does not delete build products, widget configuration, or cache data, and it preserves every other matching installed copy outside DerivedData. After any intentional widget-kind change, remove the retired desktop instance once and add the current widget from the gallery.
