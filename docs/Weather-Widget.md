...
- The small Week layout deliberately becomes a readable Today layout instead of compressing seven columns beyond legibility. Medium keeps all seven days with smaller forecast type and symbols plus wider inter-column gaps because Medium and Large share the same horizontal canvas. Large gives the full location and attribution separate header rows, then divides the remaining height between the enlarged current summary and a vertically distributed forecast section.
...
- [ ] Add Small, Medium, and Large copies and confirm Small always shows the full temperature and unit, Medium forecast columns have comfortable visible gaps, and Large shows the full location while distributing both dashboard sections through the available height.
