...
refresh_build_arguments="$(desktop_widgets_print_build_arguments "$PROJECT_ROOT/DesktopWidgets.xcodeproj" "$TEST_ROOT/DerivedData" refresh)"
assert_equal "clean" "$(/usr/bin/printf '%s\n' "$refresh_build_arguments" | /usr/bin/sed -n '1p')" "refresh should clean the previous signed products"
assert_equal "build" "$(/usr/bin/printf '%s\n' "$refresh_build_arguments" | /usr/bin/sed -n '2p')" "refresh should rebuild after cleaning"
...
[[ "$failed_provisioning_output" != *"Xcode reported success but the built app was not found"* ]] \
    || fail "installer must stop immediately after an Xcode build failure"
