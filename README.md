...
The command starts with fresh build artifacts, runs the complete `DesktopWidgetsTests` suite, builds the app and extension in Release mode, and verifies all four widget identities, all four configurable-widget schemas, Calendar's navigation actions, its Calendar View and event-indicator parameters, privacy strings, and sandbox entitlements. Calendar coverage includes Automatic and explicit family resolution, Day/Week/Month contracts, event overlap counting, disabled and permission states, month boundaries, first-weekday localization, navigation, accessibility, responsive budgets, and adaptive refresh scheduling.
